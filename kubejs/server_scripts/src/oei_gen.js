...
const performAlmostUnifiedMatching = () => {
    ...
    for (let tagId of allTags) {
        ...
        if (result !== null) {
            ...
            // 解析 material
            let material = tagId.split("/").pop()

            if (!materialResults[material]) {
                materialResults[material] = []
            }

            materialResults[material].push({
                "tag": result.tag,
    ...
    // 最后按 material 写入 json
    Object.keys(materialResults).forEach(material => {
        let jsonOutput = JSON.stringify(materialResults[material], null, 2)

        let path = `oei/${material}.json`

        FilesJS.writeFile(path, jsonOutput)

        console.log(`Material ${material} 已保存: ${path}`)
    })
