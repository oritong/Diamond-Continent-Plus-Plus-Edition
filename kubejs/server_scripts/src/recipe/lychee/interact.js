ServerEvents.recipes(e => {
    ...
    let ano_convert = [
        'gtceu:caminite_brick_dust', 'mekanism:salt'
    ]
    for (let i = 0; i < ano_convert.length - 1; i++) {
        let ano_convert_input = ano_convert[i]
        let ano_convert_output = ano_convert[i + 1]
        e.recipes.lychee.item_inside(ano_convert_input, BlockPredicate.of('oritong:glimmer')).post([Post.drop_item(ano_convert_output)]).time(5)
        e.custom({ "type": "cmr:custom_fan", "ingredients": [{ "item": ano_convert_input }, { "amount": 1000, "fluid": "oritong:glimmer", "nbt": {} }], "results": [{ "item": ano_convert_output }] })
    }
    ...
    e.recipes.lychee.block_crushing('minecraft:emerald_block').post([Post.drop_item('4x gtceu:emerald_plate')])
